						...
						<c:if test="${boardList != null}">
							<c:forEach var="e" items="${boardList}">
								<tr>
									...
									<td>${e.view_count}</td>
									<td><c:if test="${e.file_exist_yn == true}">
											<img src="#">
										</c:if></td>
								</tr>
							</c:forEach>
			...
			<div class="dataTables_paging_simple_numbers "
				...
				<ul class="pagination">
					<li class="paginate_button page-item previous disabled"
						id="btnPrevious"></li>
					<c:forEach var="i" begin="1" end="${pageMakerDto.total_page}"
						step="1">
						<li class="paginate_button page-item"><a
							href="/boardList?currentPage=${i}" class="page-link"><c:out
									value="${i}"></c:out></a></li>
					</c:forEach>
				</ul>
...
<script>
	$(document).ready(function() {
		...
		//1)start_date : 30일 전 , end_date : 오늘 세팅

		//2)if searchDto.startDate가 있는 경우 그 데이터로 값 세팅

		//3)게시판분류 및 검색조건 데이터가 있는 경우 바인딩
